...
import { ChevronDown, Calendar, AlertCircle, CheckCircle2, Loader2, User as UserIcon } from "lucide-react";
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    if (splits.length > 0 && !selectedCircuitoId) {
      // Find the first circuit where completado === false
      for (const split of splits) {
        // Assume circuits are sorted or follow seed order
        const next = split.circuitos.find((c: any) => !c.completado);
        if (next) {
          ...
          {splits.filter(s => isSplitUnlocked(s.id, splits)).map(s => (
            <button
              ...
              onClick={() => {
                ...
                const next = s.circuitos.find((c: any) => !c.completado) || s.circuitos[s.circuitos.length - 1];
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      if (cid) {
                        ...
                        const next = s?.circuitos.find((c: any) => !c.completado) || s?.circuitos[0];
                        if (next) setSelectedCircuitoId(next.id);
                      }
                    }}
                  >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="group relative bg-[#e10600] px-8 py-3 rounded-lg font-black text-xs uppercase hover:bg-red-700 transition-all shadow-xl shadow-red-900/30 overflow-hidden active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
              >
